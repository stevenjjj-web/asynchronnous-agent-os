...
The current pool manager is an in-process semaphore. It governs capacity and separates queues, but it is not a process, container, cgroup, or network namespace. Browser and code tools must name a registered sandbox adapter that declares process, container, or microVM isolation; adapters without that metadata are rejected. The adapter remains trusted code and is responsible for actually crossing into the claimed worker, container, VM, or remote execution service.
...
The gateway is bound to one configured owner tenant and rejects cross-tenant request bodies, session-key ownership mismatches, message-id ownership collisions, and object-id lookups. A bearer token represents the full trusted operator; session keys are routing identifiers, not authentication credentials. Sessions, memories, goals, events, schedules, monitors, credentials, and attention records carry tenant ownership. Agent ownership is checked by capability contracts, and each configured agent has a separate workspace root. The strong tenant boundary for this personal deployment is one OS identity, kernel process, database, configuration, credential set, and workspace set per tenant; shared-host, hostile multi-tenant isolation inside one Gateway is not claimed.

New setups require bearer authentication even on loopback. Authentication accepts only the `Authorization` header, compares secrets in constant time, and has a bounded per-address failure lockout. Query-string credentials are rejected. General API traffic has independent bounded read/write limits. The current request is admitted up to the configured limit; later requests are locked out for the configured window. The Gateway uses a fixed URL base rather than trusting the request Host header, returns generic internal errors with request IDs, and emits no permissive browser CORS policy. Startup always blocks critical security-audit findings and emits high-severity findings before binding; `NODE_ENV=production` makes high findings startup-blocking as well. Network deployments must add TLS and source-network controls at a reverse proxy.

Workspace resolution rejects absolute paths, traversal, symbolic links in every component, non-regular file reads, and mutations of core bootstrap policy files through model tools. Writes use private temporary files, `fsync`, atomic rename, and private directories. Public HTTPS reads validate every DNS answer at connection time, block private, loopback, link-local, reserved, multicast, and mapped address ranges, reject redirects and URL credentials, restrict response types and sizes, and label returned material as externally untrusted data.

External events are owner-bound. The `agent-os-event-v1` HMAC envelope covers source, timestamp, nonce, topic, correlation key, tenant, agent, and canonical payload. The replay window rejects stale messages, `(source, nonce)` uniqueness rejects replays after restart, and a reused nonce or idempotency key cannot be rebound to different content or ownership. Event matching includes tenant and agent ownership. Trusted internal scheduler, monitor, cognition, task, and listener events attach authenticated ownership metadata before entering the same durable inbox. Kernel topics such as approval resolution, goal completion, channel delivery, monitor changes, and cognition attention cannot be injected through the external event API.

Approval resolution uses one SQLite transaction for the approval decision, durable event insert, event delivery rows, and waiting-task wakeup. A crash cannot leave an approved record without its corresponding resumable event, and the Store verifies that every approval event matches the durable decision and operator authority.
...
stateDiagram-v2
  ...
  EXECUTING --> FAILED: local idempotent failure
```

The idempotency key is bound in the Store to the goal, task, tool, mode, resource pool, and canonical request; reusing it for different input is rejected even when the caller bypasses the normal tool manager. Concurrent same-process invocations with an identical key and fingerprint share one execution promise, while state transitions use compare-and-set updates to expose competing writers. A confirmed operation returns its stored result without executing again. A reconcilable uncertain operation queries the external system first. If a process disappears while a non-idempotent operation is `EXECUTING`, recovery changes it to `UNCERTAIN` and never blindly invokes it again. Tools with no idempotency, reconciliation, or compensation support are forced into the isolated side-effect pool and require explicit approval.

The I/O reactor periodically reconciles due operations that actually provide a reconciliation handler. Tool adapters may provide `prepare`, `confirm`, `reconcile`, and `compensate` handlers. Compensation is an explicit idempotency-keyed business action, not database rollback; an interrupted compensation remains visibly uncertain and is not blindly replayed. Assistant-message persistence and outbox insertion share one SQLite transaction, closing the local commit/send gap; inbound user-message and Goal/DAG creation also share one transaction. Every outbound adapter must explicitly guarantee use of the stable outbox idempotency key, and reuse of an idempotency key with different ownership or semantic content is rejected independent of JSON property order. Delivery remains at-least-once and retried from the outbox. Local file and database tools use deterministic object IDs or overwrite semantics for crash-safe replay.
...
### Memory portability and synchronization

Portable memory uses a versioned `agent-os.memory-bundle` payload and an optional `agent-os.memory-bundle-encrypted` transport envelope. The canonical payload is addressed by SHA-256 and may carry an Ed25519 signature. The signature binds the exporter identity, timestamps, complete memory records, validity, and provenance. AES-256-GCM then authenticates and encrypts the complete signed bundle. Its nonce is deterministically derived with HMAC-SHA256 from the encryption key and the complete serialized signed bundle, preserving stable content addresses without reusing a nonce when either payload or signer changes. Remote providers require both encryption and a trusted signature by default.

The outer digest addresses the encrypted storage object; the inner payload digest remains in durable sync metadata and import provenance. Decryption keys are selected by key id, so old keys can remain read-only while a new `activeKeyId` encrypts new exports. Signing keys, encryption keys, and provider bearer tokens resolve from environment or private-file references; serialized configuration never contains their runtime values.

An imported record receives a deterministic local id derived from its portable id and canonical record digest. The direct publisher remains authenticated provenance rather than part of logical memory identity. Replaying a bundle is therefore idempotent across different trusted relays, while a changed upstream record becomes a separate reviewable revision instead of silently overwriting local belief. When an imported record is exported again, the runtime restores its original portable fields and upstream provenance instead of exporting the local quarantine wrapper; this prevents two-way synchronization from generating a new duplicate at every hop. A bundle returning to its origin is matched against the original local record id.

Imports are `CANDIDATE` by default and are excluded from recall. A trusted signature proves publisher identity but does not establish semantic truth, so activation still requires an explicit operator action or an explicitly configured trusted-provider auto-activation policy. Import provenance retains outer and inner bundle digests, encryption key id, direct signer, provider, upstream id, canonical record digest, original portable metadata, and upstream provenance.

The provider boundary supports mutable local files, immutable local directory CAS, HTTPS snapshots, and HTTPS CAS. Trusted plugins may register provider adapters for protocol-specific stores such as IPFS, Arweave, S3, or enterprise storage. Plugin providers remain part of the trusted in-process plugin boundary. Remote transport uses the same bounded, DNS-pinned, no-redirect network layer as model access. CAS writes bind the digest through the object name, digest header, idempotency key, and conditional creation. Every import, export, pull, and push has a bounded-retention durable `memory_sync_runs` record.

Automatic pull and push run in the supervised `memory-sync-reactor`, not the general I/O housekeeping loop. A bounded provider worker pool prevents one slow endpoint from serializing every memory channel, while an independent heartbeat continues during network waits so the kernel does not mistake normal I/O suspension for service death. This isolates cloud latency and cancellation from task scheduling, event delivery, monitors, schedules, and the outbox. Snapshot providers may auto-pull; CAS pull always requires an explicit digest because the immutable store has no trusted concept of "latest" without a separate signed head object.
...
The first-run wizard configures agent identity, workspace, gateway exposure, model provider, credential source, resource profile, background cognition, and approval policy. Runtime-resolved model keys and gateway tokens are removed from the serialized configuration. The configuration and separate local secret file are written atomically with owner-only permissions. Every new Gateway receives a generated bearer token reference, including loopback-only deployments. Configured SecretRefs are fail-closed when unresolved.

Model-provider and public-fetch traffic uses a bounded native transport. Public endpoints must resolve entirely to public addresses, the connection is pinned to a validated result, redirects are not followed, headers and bodies are bounded, and plain HTTP is limited to loopback. Private HTTPS model access is an explicit audited opt-in through `allowPrivateNetwork`.
